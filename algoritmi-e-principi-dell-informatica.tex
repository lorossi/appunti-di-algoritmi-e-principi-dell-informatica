...
\begin{figure}[htbp]
  ...
  \centering
  \begin{tikzpicture}[auto, on grid, node distance=4cm, >=Triangle]
    \node [state](on) {\footnotesize \texttt{ON}};
    \node [state](off) [right=of on] {\footnotesize \texttt{OFF}};
...
Il \FSA in Figura~\ref{fig:esempio-funzione-transizione-parziale-FSA} non mostra la transizione effettuata in corrispondenza dell'azione di \texttt{switch\ off} in corrispondenza di stato \texttt{off} e \texttt{switch\ on} sullo stato \texttt{on}
...
\begin{itemize}
  \item L'\textbf{alfabeto}, di \(\mathcal{L}\) è composto da:
        \begin{itemize}
          \item Un insieme numerabile \textit{(finito o infinito)} di \textbf{proposizioni} (simboli di relazione nullaria): \(A, B, C, \ldots\)
          \item Un insieme di \textbf{} \(\lnot, \land, \lor, \rightarrow, \leftrightarrow\)
  ...
  \item L'\textbf{insieme di formule} di \(\mathcal{L}\) è il più piccolo insieme tale che:
        \begin{itemize}
          ...
          \item Se \(F, G\) sono formule, allora \(\lnot F\), \(F \land G\), \(F \lor G\), \(F \rightarrow G\),  \(F \leftrightarrow G\) sono \textbf{formule}
        \end{itemize}
  ...
  \item Informalmente, una \textbf{sotto formula} è una formula inclusa in un'altra formula
...
Con la notazione \(I \vDash F\) si indica che \(I\) rende vera \(F\).
...
\begin{itemize}
  \item Due formule \(F, G\) sono \textbf{semanticamente equivalenti} se e solo se vale sia \(F \vDash G\) che \(G \vDash F\) e si scrive \(F \equiv G\)
  \item Quando \(G\), sotto-formula di \(F\), viene sostituita con una formula \(H\), la formula risultante viene indicata con \(F [G \, \backslash H]\)
...
\textit{Frege} ha sviluppato la \FOL (\textbf{logica del prim'ordine} o \textbf{logica dei predicati}) nel 1879, estendendo la logica proposizionale con \textit{funzioni}, \textit{variabili} e \textit{quantificatori}.

\begin{itemize}
  \item Dal punto di vista \textit{epistemologico} (stati della conoscenza) sia \PL che \FOL considerano \textbf{verità} e \textbf{falsità} (ossia quando la logica assume i valori di \vero e \falso).
...
\begin{itemize}
  ...
  \item L'insieme di \textbf{formule della \FOL} è definito induttivamente come il più piccolo insieme tale che:
        \begin{itemize}
          ...
          \item se \(F, G\) sono formule e \(X\) è una variabile, allora anche
                \[\lnot F \quad F \land G \quad F \lor G \quad F \rightarrow G \quad F \leftrightarrow G \quad \exists \, X F \quad \forall \, X F \] sono formule
        \end{itemize}
  \item I \textbf{letterali} sono atomi (letterali positivi) o atomi negati (letterali negativi)
  \item Le \textbf{parentesi sono omesse} ovunque possibile usando l'ordine di precedenza
...
\paragraph{Esempi di traduzioni in \FOL}

\begin{itemize}
  \item \inlinequote{tutti gli esseri umani sono mortali} diventa \[\forall \,X  \, (h(X) \rightarrow m(X))\]
  \item \inlinequote{ogni bambino ha dei genitori} diventa \[\forall \,  X \,\left(c(X) \rightarrow \left(\exists \, Y \, \exists \, Z p(X, Y, Z)\right)\right)\]
\end{itemize}
...
\begin{itemize}
  \item Il connettivo principale usato con \(\forall\) è \(\rightarrow\)
        \begin{itemize}
          ...
          \item notare che la formula \[ \forall \, X \, (at(X, polimi) \land smart(X)) \] significa che tutti sono al Polimi e tutti sono brillanti
        \end{itemize}
  \item Analogamente, il connettivo principale da usare con \(\exists\) è \(\land\)
        \begin{itemize}
          ...
          \item notare che la formula \[ \forall \, X \, (at(X, polimi) \rightarrow smart(X)) \] significa che c'è qualcuno che o non è al Polimi o è brillante (o entrambe)
        \end{itemize}
\end{itemize}
...
Una interpretazione \(\mathcal{I}\) di un alfabeto \(\mathcal{A}\) è un \textbf{dominio} non vuoto \(D\) (indicato anche come \(|I|\)) a una funzione che associa:
...
La logica può aiutare a descrivere i linguaggi: gli insiemi di stringhe, infatti, possono essere visti come abbreviazioni di formule \FOL.
...
\bigskip
Una volta che la lingua viene definita in modo logico, occorrerebbe definire tutti i predicati e le funzioni non elementari, quali \(=, >, +, -, \times, \ast, \cdot, \ldots\).
Per evitare di doverlo fare, si fa normalmente riferimento a logiche più ristrette, con sintassi specifica già predisposta.
...
Soprattutto, tra di questi si riconosce la \FOL, che può essere usata per questo scopo.
...
Quando qualche condizione è eliminata dalla precondizione, la specifica diventa insoddisfacente.
...
% Dei possibili valori di \(g\), \(h\) e \(f\) sono mostrati nella Figura~\ref{fig:possibili-valori-fgh}.

% \begin{figure}[htbp]
%   \bigskip
%   \centering
%   \tikzfig[0.65]{image-18.tikz}
%   \caption{Possibili valori di \(g\), \(h\) e \(f\)}
%   \label{fig:possibili-valori-fgh}
%   \bigskip
% \end{figure}
...
\paragraph{Dimostrazione del lemma 1}
...
\paragraph{Dimostrazione lemma 3}
...
Un'esempio di tale linguaggio è mostrato nella Tabella~\ref{tab:instruction-set-e-semantica}.
...
\begin{table}[htbp]
  ...
  \begin{minipage}[t]{.45\textwidth}
    ...
    \begin{tabular}{ll}
      ...
      \texttt{JZZ l}      & \(\texttt{PC} \leftarrow \texttt{l} \text{ se } \texttt{N}[0] < 0\) \\
    \end{tabular}
  \end{minipage}
  \bigskip
  \caption{Instruction set e semantica pseudo-\textit{RTL}}
  \label{tab:instruction-set-e-semantica}
\end{table}
...
\section{Concetti base di complessità degli algoritmi}
