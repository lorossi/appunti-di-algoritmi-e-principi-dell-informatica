...
% TODO sistemare la dimensione dell'array, i rettangoli sono troppo grossi
\begin{figure}[htbp]
  \bigskip
  \centering
...
\subsection{Alberi binari di ricerca - \textit{Binary Search Trees}}
...
Sia \(x\) un nodo di un \BST.
Se \(y\) è parte del:

\begin{itemize}
  \item Sotto albero \textbf{sinistro} di \(x\), allora \(key[y] \leq key[x]\)
  \item Sotto albero \textbf{destro} di \(x\), allora \(key[x] \leq key[y]\)
\end{itemize}

Più semplicemente, il figlio sinistro di un nodo sarà sempre \textit{minore o uguale} del figlio destro dello stesso nodo.
Allo stesso modo, il figlio destro sarà \textit{maggiore o uguale} di quello sinistro.

\bigskip
L'altezza di un \BST è, per ogni \(n \geq 1\), è pari a \(h \leq \log{\left(\dfrac{n+1}{2}\right)}\).
...
Nei Paragrafi seguenti \textit{(\ref{par:ricerca-bst}~\ref{par:predecessore-successore-bst})} verranno analizzate delle operazioni comunemente applicate ai \BST, insieme alle loro relative complessità.
...
\paragraph{Predecessore e successore}
\label{par:predecessore-successore-bst}
