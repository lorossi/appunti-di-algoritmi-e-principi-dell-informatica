...
Questa è una tecnica di natura ricorsiva, in quanto i problemi più piccoli sono risolti tramite l'algoritmo stesso.
...
\begin{itemize}
  ...
  \item se il sotto problema ha dimensione \(n\) sufficientemente piccola \textit{(tale per cui \(n < c\) con \(c\) costante)} allora esso può essere risolto in tempo costante \textit{(quindi in \(\Theta(1)\))}
...
Questo metodo fornisce un risultato approssimato la cui validità va provata con altre tecniche \textit{(normalmente per sostituzione)}.
...
\begin{lstlisting}[style=pseudocode, caption={Costruzione di un max-heap}, label={lst:costruzione-max-heap}]
MAX-HEAPIFY(A, i)
  l := LEFT(i)
  r := RIGHT(i)
...
\begin{lstlisting}[style=pseudocode, caption={Algoritmo di costruzione di un max-heap}, label={lst:algoritmo-build-max-heap}]
BUILD-MAX-HEAP(A)
...
\begin{minipage}{0.99\textwidth}
  \begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo di Merge Sort}, label={lst:pseudocodice-merge-sort}]
MERGE-SORT(A, p, R)
  if p < r
    q := floor((p + r) / 2)
    MERGE-SORT(A, p, q)
...
\end{lstlisting}
  ...
  \begin{minipage}{0.99\textwidth}
    ...
    \begin{minipage}[t]{0.495\textwidth}
      \begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo di Merge}, label={lst:pseudocodice-merge}]
MERGE(A, p, q, r)
  n1 := q - p + 1
  n2 := r - q
  alloca L[1 .. n1+1]
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo Heapsort}, label={sec:algoritmo-heapsort}]
HEAPSORT(A)
  BUILD-MAX-HEAP(A)
...
La parte \inlinequote{complicata} da implementare riguarda la partizione: essa viene effettuata tramite l'algoritmo \texttt{PARTITION},che avrà l'incarico di scegliere un elemento di \textit{pivot}.
...
\begin{center}
  \begin{minipage}[t]{0.495\textwidth}
    \begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo QUICKSORT}, label={sec:algoritmo-quicksort}]
QUICKSORT(A, p, r):
  if p < r:
    q := PARTITION(A, p, r)
    QUICKSORT(A, p, q - 1)
  ...
  \end{minipage}
  \begin{minipage}[t]{0.495\textwidth}
    \begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo PARTITION}, label={sec:algoritmo-partition}]
PARTITION(A, p, r):
    x := A[r]
    i := p - 1
    for j := p to r - 1
...
\begin{itemize}
  \item Nel caso ottimo, la ricorrenza è \(T(n) \leq 2T(\sfrac{n}{2}) + \Theta(n)\)
        \begin{itemize}
          \item la soluzione porterà a \(T(n) = \bigO\left(n \log{(n)}\right)\)
        \end{itemize}
  \item Nel caso pessimo, in cui l'algoritmo produce una divisione \(9\) a \(1\), la ricorrenza è \(T(n) \leq T(\sfrac{9n}{10}) + T(\sfrac{n}{10}) + c \cdot n\)
        \begin{itemize}
          \item la soluzione porterà a \(T(n) = \bigO\left(n^2\right)\)
        \end{itemize}
\end{itemize}
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice algorimo Counting Sort}, label={lst:counting-sort}]
COUNTING-SORT(A, B, K):
  for i := 0 to k
    C[i] := 0
  for j := 1 to A.length
...
\begin{minipage}[t]{0.495\textwidth}
  \begin{lstlisting}[style=pseudocode, caption={Push in pila}, label={lst:push-pila}]
PUSH(S, x)
  if S.top == S.length
    error "overflow"
  else S.top := S.top + 1
...
\end{minipage}
\begin{minipage}[t]{0.495\textwidth}
  \begin{lstlisting}[style=pseudocode, caption={Pop in pila}, label={lst:pop-pila}]
POP(S)
  if S.top == 0
    error "underflow"
  else S.top := S.top - 1
...
\begin{minipage}[t]{0.495\textwidth}
  \begin{lstlisting}[style=pseudocode, caption={Enqueue in coda}, label={lst:enqueue-coda}]
ENQUEUE(Q, x)
    if Q.tail = Q.length
      Q.tail := 1
...
\end{minipage}
\begin{minipage}[t]{0.495\textwidth}
  \begin{lstlisting}[style=pseudocode, caption={Dequeue in coda}, label={lst:dequeue-coda}]
DEQUEUE(Q)
    x := Q[Q.head]
    if Q.head = Q.tail
      Q.head := 1
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo List Search}, label={lst:list-search}]
LIST-SEARCH(L, k)
  x := L.head
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo List Insert}, label={lst:list-insert}]
LIST-INSERT(L, x)
  x.next := L.head
  if L.head != NIL
    L.head.prev := x
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo List Delete}, label={lst:list-delete}]
LIST-DELETE(L, x)
  if x.prev != NIL
    x.prev.next := x.next
  else
...
\begin{lstlisting}[style=pseudocode, caption={Eliminazione}, label={lst:delete-dizionario}]
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo di inserimento}, label={sec:inserimento-ind-aperto}]
HASH-INSERT(T, k)
i := 0
repeat
  j := h(k, i)
...
\begin{lstlisting}[style=pseudocode, caption={Pseudocodice dell'algoritmo di ricerca}, label={sec:ricerca-ind-aperto}]
HASH-SEARCH(T, k)
  i := 0
  repeat
    j := h(k, i)
...
Nei Paragrafi seguenti \textit{(\ref{attraversamento-albero-binario}~-~\ref{par:eliminazione-albero-binario})} verranno analizzate delle operazioni comunemente applicate agli alberi binari, insieme a delle immagini esplicative.
...
\label{par:eliminazione-albero-binario}

L'eliminazione di un elemento da un nodo non è sempre possibile senza creare ambiguità: se un nodo con due sotto alberi viene eliminato, non sarà possibile costruire un nuovo albero binario.
...
\bigskip
Il processo di eliminazione è illustrato nella Figura~\ref{fig:eliminazione-albero-binario}.

\begin{figure}[htbp]
  \bigskip
  \centering
  \tikzfig[0.8]{image-39.tikz}
  \caption{Eliminazione di un nodo in un albero binario}
  \label{fig:eliminazione-albero-binario}
  \bigskip
\end{figure}
...
Nei Paragrafi seguenti \textit{(\ref{par:ricerca-bst}~\ref{par:eliminazione-bst})} verranno analizzate delle operazioni comunemente applicate ai \BST, insieme alle loro relative complessità.
...
\paragraph{Eliminazione}
\label{par:eliminazione-bst}

Analogamente a quanto già descritto nell'inserimento, l'operazione di eliminazione causa modifiche nella struttura del \BST.
...
Lo pseudocodice dell'algoritmo è mostrato nel Listato~\ref{lst:eliminazione-bst}, mentre una raffigurazione dei tre casi è mostrata nelle Figure \ref{fig:eliminazione-nodo-bst-senza-sotto-alberi}, \ref{fig:eliminazione-nodo-bst-con-un-solo-sotto-albero} e \ref{fig:eliminazione-nodo-bst-con-due-sotto-alberi}.

\begin{lstlisting}[style=pseudocode, caption={Eliminazione di un nodo}, label={lst:eliminazione-bst}]
TREE-DELETE(T, z):
...
\begin{figure}[htbp]
  ...
  \caption{Eliminazione di un nodo senza sotto alberi}
  \label{fig:eliminazione-nodo-bst-senza-sotto-alberi}
  \bigskip
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Eliminazione di un nodo con un solo sotto albero}
  \label{fig:eliminazione-nodo-bst-con-un-solo-sotto-albero}
  \bigskip
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Eliminazione di un nodo con due sotto alberi}
  \label{fig:eliminazione-nodo-bst-con-due-sotto-alberi}
  \bigskip
\end{figure}
...
\begin{itemize}
  ...
  \item Logaritmo del rapporto: \(\log_a{\left(\dfrac{b}{c}\right)} = \log_a{(b)} - \log_a{(c)}\)
